...
Suuri vaikutus koodin tehokkuuteen on kääntäjällä, joka kääntää tietyllä ohjelmointikielellä kirjoitetun koodin konekielelle tietokoneen prosessorin suoritettavaksi. Kääntäjän tehtävänä on saada aikaan konekielinen koodi, jonka toiminta vastaa alkuperäistä koodia ja joka lisäksi toimii tehokkaasti.
...
Tutustumme tarkemmin konekielen toimintaan seuraavasta luvusta alkaen. Tässä luvussa on joitakin konekielisiä esimerkkejä, mutta ei haittaa, vaikka et ymmärtäisi vielä niiden toimintaa kaikilta osin.
</div>
...
TODO kääntäjän optimointiliput

### Ohjelmointikieli ja tehokkuus

C-kielellä on maine tehokkaana ohjelmointikielenä, mutta mitä tarkoittaa, että jokin ohjelmointikieli on tai ei ole tehokas? Oikeastaan on tarkempaa sanoa, että on olemassa kääntäjiä, jotka pystyvät kääntämään C-kielisestä koodista tehokasta konekielistä koodia eri prosessoreja varten.
...
Tällainen muutos olisi kuitenkin varsin vaikea tehdä niin, että koodi toimisi varmasti samalla tavalla kaikissa tilanteissa. Esimerkiksi tässä pitäisi ottaa huomioon tilanne, jossa `n` on negatiivinen, jolloin äskeinen kaava ei anna samaa tulosta kuin alkuperäinen silmukka. Lisäksi pitäisi ottaa huomioon tilanne, jossa `n` on niin suuri, että summakaavassa tulee ylivuoto.
</div>
...
Silmukan _purkaminen_ (_unrolling_) on koodin optimointitekniikka, jossa silmukkaa muutetaan niin, että jokainen uuden silmukan kierros vastaa useampaa alkuperäisen silmukan kierrosta. Tavoitteena on vähentää silmukan kierrosten määrää ja käyttää vähemmän konekielisiä komentoja silmukan hallinnointiin.
...
Funktion kutsumista koodissa voidaan tehostaa funktion _upotuksella_ (_inlining_) eli korvaamalla funktionkutsu koodilla, jonka funktio suorittaa sitä kutsuttaessa. Esimerkiksi koodi
...
Moderneissa prosessoreissa on monenlaisia ominaisuuksia, jotka tehostavat koodin suoritusta. Prosessorit suorittavat useita komentoja rinnakkain ja käyttävät välimuisteja, jotka nopeuttavat muistin käsittelyä.

Asiaa voi ajatella siltä kannalta, että prosessorit pyrkivät _ennakoimaan_ koodin toimintaa yhtä komentoa pidemmälle ja suorittamaan koodin kokonaisuutena mahdollisimman tehokkaasti. Jos koodissa on peräkkäin komentoja, jotka eivät vaikuta toisiinsa, prosessori voi suorittaa ne yhtä aikaa. Jos koodi lukee muistia tietystä kohdasta, välimuistiin voidaan hakea valmiiksi seuraavan kohdan sisältö siltä varalta, että koodi lukee myöhemmin senkin.
...
Kun koodi käyttää viittaustapaa `[i][j]`, se käy tämän muistialueen sisällön alkio kerrallaan vasemmalta oikealle, mikä on välimuistin kannalta ihanteellinen tapa käsitellä muistia. Viittaustavassa `[j][i]` koodi puolestaan tekee muistissa `N` alkion pituisia hyppyjä, mikä sopii huonosti yhteen välimuistin kanssa.
...
Toisaalta viittaustavassa `[j][i]` koodi toimii yllättäen nopeammin alkiotyypillä `long` kuin `int`. Tämä ero ei luultavasti liity välimuistiin vaan johonkin muuhun asiaan muistin käsittelyssä. Jostain syystä koodi pystyy tässä tilanteessa viittaamaan tehokkaammin muistissa olevaan `long`-arvoon kuin `int`-arvoon.
