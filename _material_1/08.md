...
Koodin tehokkuuteen vaikuttavat koodin toimintalogiikka, koodin toteutuksen yksityiskohdat sekä koodin suoritustapa konekielen tasolla.
...
Koodin tehokkuuden kannalta on myös hyvä tiedostaa, mitkä asiat _eivät_ vaikuta tehokkuuteen. Näitä ovat erilaiset koodin muotoiluun ja nimeämiseen liittyvät asiat, jotka eivät välity lopulliseen konekieliseen koodiin.

Tyhjällä tilalla ja kommenteilla ei ole merkitystä tehokkuuden kannalta, koska näillä asioilla ei ole vaikutusta koodin toimintaan. Esimerkiksi koodi `if(x==42)` ei ole tehokkaampi kuin koodi `if (x == 42)`.

Koodin tehokkuuteen eivät vaikuta lyhennysmerkinnät, jotka tarkoittavat samaa kääntäjälle. Esimerkiksi lausekkeet `a++`, `a += 1` ja `a = a + 1` ovat yhtä tehokkaita, koska ne voidaan esittää samalla tavalla konekielessä.

Koodia ei myöskään nopeuta esimerkiksi, että muuttujan `item_count` sijasta käytetään muuttujaa `x`. Muuttujan nimen pituus ei vaikuta tehokkuuteen, koska konekielessä ei ole muuttujien nimiä, vaan muistiin viitataan osoitteilla.
