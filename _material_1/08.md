...
Testataan koodia parametreilla `T` = 500 ja `N` = 3000 pienin muunnoksin:
...
Toisaalta viittaustavassa `[j][i]` koodi toimii hieman yllättäen nopeammin alkion tyypin ollessa `long` kuin `int`. Tämä ero ei luultavasti liity välimuistiin vaan johonkin muuhun muistin käsittelyssä. Jostain syystä koodi pystyy tässä tilanteessa viittaamaan tehokkaammin muistissa olevaan `long`-arvoon kuin `int`-arvoon.
