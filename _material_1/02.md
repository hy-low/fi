...
Rakenteisen tietotyypin kentät tallennetaan muistiin periaatteessa samalla tavalla kuin yksittäiset muuttujat. Esimerkiksi äskeisen tyypin `struct Position` sisältö voisi olla muistissa näin:
...
Tässä kentät vievät yhteensä tilaa 5 tavua (`int` vie tilaa 4 tavua ja `char` vie tilaa 1 tavun), mutta `sizeof(struct Test)` antaakin yleensä tuloksen 8.

Tässä on kyse siitä, että rakenteisen tietotyypin muistialueen koko on yleensä moninkerta suurimman kentän koosta. Tässä suurimman kentän koko on 4 tavua ja tämän moninkerta on 8 tavua.
...
Union-tyypissä kenttien sisältö tallennetaan _päällekkäin_ muistiin. Esimerkiksi seuraava koodi määrittelee tyypin `union Test`, jossa on päällekkäin `int`-muuttuja `a` ja `float`-muuttuja `b`.
