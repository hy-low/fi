...
Käytännössä nykyään yleensä C-kielen `int`-tyyppi on 32-bittinen ja `long`-tyyppi on joko 32-bittinen (Windows) tai 64-bittinen (Linux, Mac).
</div>
...
Binäärilukua indeksoidaan tavallisesti oikealta vasemmalle nollasta alkaen pienimmästä 2:n potenssista suurimpaan. Esimerkiksi yllä olevassa binääriluvussa on ykkösbitit kohdissa 0, 1, 3, 4, 5 ja 6. Luvun eniten merkitsevä bitti on kohdassa 31 ja vähiten merkitsevä  bitti on kohdassa 0. Vastaavasti eniten merkitsevä tavu on 00000000 ja vähiten merkitsevä tavu on 01111011.
...
<div class="note" markdown="1">
Vaikka kahden komplementti on yleisesti käytetty tapa negatiivisten kokonaislukujen esittämiseen, C-standardi on sallinut perinteisesti muitakin esitystapoja. Kuitenkin uusin C23-standardi määrittelee, että negatiiviset kokonaisluvut esitetään aina kahden komplementin avulla.
</div>
...
Tarkkaan ottaen C-standardi ei määrittele, mitä tapahtuu etumerkillisen luvun ylivuodossa. Useimmissa ympäristöissä luku pyörähtää ympäri yllä olevalla tavalla, mutta periaatteessa voisi tapahtua jotain muutakin. Sen sijaan standardi määrittelee, että etumerkittömän luvun ylivuodossa luku pyörähtää ympäri.
...
Usein kuitenkin struct-muuttujaa käsitellään osoittimen kautta, jos ei ole erityisesti tarvetta tehdä kopiota sen sisällöstä. Tämä voi tehostaa muuttujan käsittelyä, koska muuttujan kenttiä ei tarvitse kopioida vaan riittää välittää osoitin muuttujaan.

Seuraavassa koodissa `p` on osoitin, joka osoittaa muuttujaan `a`. Koodi muuttaa muuttujan `a` kentän `x` sisältöä osoittimen `p` kautta. Syntaksi `p->x` tarkoittaa samaa kuin `(*p).x` eli se viittaa kentän sisältöön osoittimen kautta.
...
Osoitinta voidaan käyttää myös, kun struct-muuttuja annetaan funktion parametriksi. Esimerkiksi seuraavassa on aiemman funktion `distance` uusi toteutus, jossa parametreja ei kopioida vaan ne välitetään osoittimina.
...
Tämä muutos voi tehostaa funktion toimintaa, koska funktiolle ei välitetä kopioita struct-muuttujista vaan vain osoittimet.

<div class="note" markdown="1">
Tässä tapauksessa `struct Position` sisältää vain kaksi `int`-kenttää eli sen sisältö vie vähän tilaa. Tämän takia ei ole käytännössä merkitystä, välitetäänkö tieto kopiona vai osoittimen avulla. Kuitenkin tilanne muuttuisi, jos struct-tyyppi sisältäisi paljon tietoa, jolloin sitä olisi hidasta kopioida.
</div>
...
Tässä `sizeof(struct Test)` on 6, koska olemme ohjeistaneet kääntäjää pakkaamaan kentät tiiviisti ilman kohdistusta. Tämä on gcc-kääntäjän erikoissyntaksi, joka ei toimi välttämättä muissa kääntäjissä.

![](../img/struct5.svg)

Vaikka kenttien pakkaaminen säästää tilaa, tämä ei ole koodin tehokkuuden kannalta hyvä ratkaisu, koska kenttää `b` ei ole kohdistettu oikealla tavalla.
