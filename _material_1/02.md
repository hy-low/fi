...
C-kielen perustietotyypit ovat kokonaisluvut `char`, `short`, `int` ja `long` sekä liukuluvut `float`, `double` ja `long double`. Jokaisella tietotyypillä on kiinteä koko, joka vaikuttaa siihen, miten suuren tai tarkan luvun sillä pystyy esittämään.

Pienin tietotyyppi on `char`, jonka kokona on 1 tavu (8 bittiä). Jokaisen muun tietotyypin koko on jokin moninkerta tavuja.

C-standardi ei määrittele tarkasti, miten suuria eri tietotyypit ovat. Esimerkiksi
tyypillisessä Linux-ympäristössä `int`-tyypin koko on 4 tavua (32 bittiä) ja `long`-tyypin koko on 8 tavua (64 bittiä), mutta ohjelmoija ei voi luottaa siihen, että näin olisi kaikissa ympäristöissä. C-standardi määrittelee vain, että `int`-tyypin koko on _ainakin_ 2 tavua ja `long`-tyypin koko on _ainakin_ 4 tavua.
...
Avainsana `struct` määrittelee rakenteisen tietotyypin, joka muodostuu nimetyistä kentistä. Rakenteista tietotyyppiä voi käyttää koodissa varsin samalla tavalla kuin yksittäisen lukuarvon sisältävää perustietotyyppiä.
...
Seuraava koodi määrittelee muuttujat `p1` ja `p2`, joiden tyyppinä on `struct Position`. Koodissa on myös funktio `distance`, joka laskee pisteiden etäisyyden.
...
Koodi laskee etäisyyden Pythagoraan lauseella käyttäen neliöjuurifunktioa `sqrt`, joka on määritelty standardikirjaston otsikkotiedostossa `math.h`. Tämän otsikkotiedoston kanssa käännöskomennossa tulee olla mukana lippu `-lm`:
...
Rakenteisen tietotyypin sisältö kopioidaan muuttujan sijoituksessa ja funktion parametrina. Esimerkiksi seuraava koodi kopioi muuttujan `a` sisällön muuttujaan `b` ja muuttaa sen jälkeen muuttujan `b` kenttää.
...
Seuraavassa koodissa `p` on osoitin eikä muuttujan `a` sisältöä kopioida. Merkintä `->` on lyhyt tapa viitata tietotyypin kenttään osoittimen kautta. Tässä merkinnät `p->x` ja `p->y` tarkoittavat samaa kuin `(*p).x` ja `(*p).y`.
...
Osoitinta voidaan käyttää välittämään tietotyypin sisältö _tehokkaammin_ funktion parametrina ilman kopiointia. Seuraavassa on aiempi funktio `distance` muutettuna niin, että sen parametrit ovat osoittimia:
...
Tämän muutoksen jälkeen funktiota voi kutsua näin:
...
Rakenteisen tietotyypin kenttien sisällöt tallennetaan muistiin samassa järjestyksessä kuin kentät esiintyvät tietotyypin määrittelyssä. Esimerkiksi äskeisen tyypin `struct Position` sisältö voisi olla muistissa näin:

![](../img/struct1.svg)

Tässä tilanne on selkeä: `sizeof(struct Position)` antaa tuloksen 8, koska tietotyypissä on kaksi `int`-tyyppistä kenttää, jotka on sijoitettu peräkkäin muistissa ja kumpikin kenttä vie tilaa 4 tavua.

Kuitenkin kenttien sijoittelu muistissa muuttuu monimutkaisemmaksi, jos tietotyypin osana on eri kokoisia kenttiä. Esimerkiksi:
...
Tässä kentät vievät tilaa 5 tavua (`int` vie tilaa 4 tavua ja `char` vie tilaa 1 tavun), mutta `sizeof(struct Test)` antaakin tyypillisesti tuloksen 8. Tilanne muistissa voisi näyttää seuraavalta:

![](../img/struct2.svg)

Muistialueen alussa kentät vievät tilaa 5 tavua, mutta lisäksi on varattu 3 täytetavua, jolloin muistialueen kooksi tulee yhteensä 8 tavua.

Seuraavassa on vielä toinen esimerkki, jossa kentät vievät tilaa 6 tavua (1 + 4 + 1 = 6) mutta muistia kuluukin 12 tavua. Tässä tapauksessa kummankin `char`-kentän jälkeen on varattu 3 täytetavua.
...
Tässä on kyse siitä, että kääntäjä varaa tietotyypille muistia seuraavasti:

- Kunkin kentän muistiosoitteen tulee olla jaollinen kentän koolla. Esimerkiksi `int`-kentän koko on 4 tavua, joten sen tulee sijaita 4:llä jaollisessa osoitteessa.

- Muistialueen yhteiskoon tulee olla jaollinen suurimman kentän koolla. Esimerkiksi kun `int`-kenttä on suurin, yhteiskoon tulee olla jaollinen 4:llä.

Tämä tapa varata muistia tehostaa tietotyypin käsittelyä, koska tietokoneen prosessori pystyy käsittelemään tietoa tehokkaammin, kun muistiosoite on jaollinen tiedon koolla eli tieto on _kohdistettu_ oikealla tavalla muistissa. Ylimääräisen muistin käyttäminen siis tehostaa koodin toimintaa.

<div class="note" markdown="1">
Joskus kentät saa mahdutettua pienemään tilaan järjestämällä ne toisella tavalla. Esimerkiksi voisimme järjestää äskeisen tietotyypin kentät näin:
...
Tämän seurauksena muistia kuluu vain 8 tavua aiemman 12 tavun sijasta, koska kentät voidaan nyt sijoittaa muistiin näin:

![](../img/struct4.svg)

Voimme myös ohjeistaa `gcc`-kääntäjää sijoittamaan kentät peräkkäin muistissa ilman kohdistamista seuraavasti:
...
Nyt muistia kuluu tarkasti 6 tavua ja tilanne näyttää seuraavalta:

![](../img/struct5.svg)

Tämä on kuitenkin tehokkuuden kannalta huono ratkaisu, koska tietotyypin sisältö muistissa ei ole enää oikealla tavalla kohdistettu.
</div>
