...
C-kielen taulukoissa ja muistin käsittelyssä on perusperiaatteena, ettei koodi tee ohjelmoijalta salassa mitään raskasta, kuten käy läpi koko taulukon sisältöä tai kopioi sitä toiseen paikkaan muistissa.

Esimerkiksi taulukon sisältöä ei voi kopioida syntaksilla `a = b` eikä taulukoiden sisältöä voi vertailla syntaksilla `a == b`. Samoin funktion kutsussa välitetään osoitin taulukon alkuun eikä kopioida taulukon sisältöä. Yleensä ottaen jos koodin tulee tehdä jotain taulukon läpikäyntiä vaativaa, tämä tulee toteuttaa joko itse silmukan avulla tai käyttää standardikirjaston funktiota.

<div class="note" markdown="1">
Yksi poikkeus on tilanne, jossa taulukko on rakenteisen tietotyypin sisällä:
...
Tietotyypin `struct Array` sisällön voi kopioida syntaksilla `a = b` ja välittää funktion parametrina siitä huolimatta, että sen sisällä on taulukko.

Tämä on tavallaan C-kielen porsaanreikä, joka johtuu siitä ristiriidasta, että rakenteinen tietotyyppi käyttäytyy samalla tavalla kuin perustietotyypit, mutta toisaalta sen sisällä voi olla taulukko kenttänä.

C-kielessä ei kuitenkaan ole hyvää ohjelmointityyliä laittaa taulukkoa tietotyypin sisään sen takia, että sitä pystyisi kopioimaan.
</div>
