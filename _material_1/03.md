...
Tässä taulukon kooksi ilmoitetaan 400, koska taulukossa on 100 alkiota ja jokainen alkio vie tilaa 4 tavua.
</div>
...
Tiedostossa `stdlib.h` on kaksi kätevää algoritmia taulukoiden käsittelyyn:

- Funktio `qsort` järjestää taulukon sisällön.
- Funktio `bsearch` etsii järjestetystä taulukosta annettua alkiota.

C-kielessä tällaisten algoritmien toteutuksessa on haasteena, kuinka toteuttaa algoritmit yleisesti niin, että ne soveltuvat eri tyyppisten taulukoiden käsittelemiseen. C-kielessä ei ole suoraan mahdollista määritellä esimerkiksi, että funktion parametrina on minkä tahansa tyyppinen taulukko. Kuitenkin asia on ratkaistavissa käyttämällä `void`-tyyppistä osoitinta sekä erikseen määriteltävää vertailufunktiota, johon annetaan viittaus osoittimena.

### Järjestäminen

Funktio `qsort` järjestää annetun taulukon sisällön. Funktion määrittely on seuraava:
...
Funktio on pyritty tekemään yleiseksi niin, että sillä voi järjestää eri tyyppisiä taulukoita eri tavoilla. Tämän takia parametrissa `base` ja funktion `cmp` parametreissa on tyyppi `void*`, joka viittaa osoittimeen, jonka tyyppiä ei ole määritelty.
...
Funktio `bsearch` etsii tehokkaasti alkion järjestetystä taulukosta. Funktion määrittely on seuraava:
