...
Taulukko on C-kielen perustietorakenne, joka muodostuu peräkkäisistä muistissa olevista alkioista. Taulukko on tehokas matalan tason tietorakenne, joka vastaa suoraan tiedon tallennustapaa tietokoneen muistissa.

Taulukko on melko rajoittunut tietorakenne, koska sen kokoa ei voi muuttaa määrittelyn jälkeen. Tämä on tärkeä ero verrattuna muuttuvan kokoiseen listaan, joka on esimerkiksi Python-kielen perustietotyyppi. Toisaalta taulukon avulla voi toteuttaa muuttuvan kokoisen listan, kuten teemme luvussa 5.

## Taulukon yleiskuva

### Taulukon määrittely

Taulukon määrittelyssä ilmoitetaan taulukon alkioiden tyyppi ja määrä sekä mahdollisesti myös taulukon alkusisältö. Esimerkiksi seuraava koodi määrittelee taulukon, joka sisältää 8 alkiota ja kukin alkio on `int`-tyyppinen luku:

```c
int numbers[8];
```

Tässä tapauksessa taulukon alkusisältöä ei ole annettu, jolloin se määräytyy samalla tavalla kuin muuttujan määrittelyssä:

- Jos taulukko on määritelty globaalisti päätasolla, taulukon jokaisen alkion alkuarvoksi tulee automaattisesti 0.

- Jos taulukko on määritelty paikallisesti funktiossa, taulukko on alustamaton eli sen alkusisältönä voi olla mitä tahansa muistissa ollutta tietoa.
...
Java-kielessä taulukkomuuttuja ei viittaa kiinteään muistiosoitteeseen, vaan muuttuja vastaa enemmänkin C-kielen osoitinta. Esimerkiksi seuraava Java-koodi asettaa muuttujat `a` ja `b` viittaamaan samaan taulukkoon:
...
Perusperiaate C-kielessä on, ettei kielen syntaksi tee ohjelmoijalta salassa mitään raskasta, kuten taulukon kopiointia tai läpikäyntiä. Jos jokin C-kielen operaatio _näyttää_ kevyeltä, se myös yleensä on kevyt. Toinen tilanne on esimerkiksi Python-kielessä, jossa listaoperaatioissa `a == b`, `a[1:]` ja `x in a` on kevyt syntaksi mutta operaatiot ovat raskaita, koska ne käyvät läpi listan sisällön.
...
Esimerkiksi kun `c` sisältää merkin `'2'`, tässä lasketaan `'2' - '0'`, jonka tuloksena on 2. Esimerkiksi ASCII-merkistössä merkin `'0'` merkkikoodi on 48 ja merkin `'2'` merkkikoodi on 50, jolloin lasketaan 50 – 48 = 2.
...
Tässä formaatti `%s` tarkoittaa, että luetaan merkkijono, joka päättyy tyhjään tilaan kuten rivinvaihtoon tai välilyöntiin.
...
- `key` on osoitin haettavaan alkioon
