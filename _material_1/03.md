...
C-kielessä ei ole syntaksia taulukon sisällön käsittelyyn kokonaisuutena. Esimerkiksi lauseke `a = b` ei kopioi taulukon `b` sisältöä taulukkoon `a` eikä lauseke `a == b` vertaile taulukoiden `a` ja `b` sisältöä. Myöskään funktiokutsussa taulukon sisältöä ei kopioida vaan funktio saa osoittimen taulukkoon.
...
Tässä tapauksessa struct-muuttujan sisällön voi kopioida syntaksilla `a = b` ja välittää funktion parametrina siitä huolimatta, että sen sisällä on taulukko. Esimerkiksi:
