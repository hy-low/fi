...
Kullakin konekielen komennolla on tietty määrä _operandeja_, joita voi ajatella komennon parametreina. Esimerkiksi komennolla `mov` on kaksi operandia: ensimmäinen operandi ilmaisee, minne tietoa kopioidaan, ja toinen operandi ilmaisee, mistä tietoa kopioidaan.

Tietokoneen prosessorille äskeinen koodi esitetään tavujonona `48 89 c3 48 ff c0`, jossa tavut `48 89 c3` vastaavat komentoa `mov rbx, rax` ja tavut `48 ff c0` vastaavat komentoa `inc rax`. Jokaiselle konekielen komennolle on tietty tavuesitys, jonka tietokoneen prosessori pystyy tulkitsemaan. Ihmiselle koodin kirjoittaminen ja lukeminen tässä muodossa olisi vaivalloista.
...
`48 be 00 20 40 00 00 00 00 00` | `48 be` <br> (siirrä 64-bittinen vakioarvo rekisteriin `rsi`) | `00 20 40 00 00 00 00 00` <br> (muistiosoite `0x402000`, joka osoittaa tulostettavan merkkijonon alkuun)
...
Syynä tähän on x86-konekielen ominaisuus: kun 32-bittiseen rekisteriin sijoitetaan arvo, vastaavan 64-bittisen rekisterin toinen puoli nollautuu automaattisesti. Esimerkiksi kun rekisteriin `eax` sijoitetaan arvo `01 00 00 00`, rekisterin `rax` arvoksi tulee `01 00 00 00 00 00 00 00`.

Tällainen muutos käännöksessä on järkevä, koska koodin toiminta ei muutu mutta sen tavuesityksen koko pienenee. Nyt 32-bittinen luku vie tilaa koodissa 4 tavua, kun taas 64-bittinen luku olisi vienyt 8 tavua.
