...
Vaikka C-kieli on matalan tason ohjelmointikieli, se kätkee paljon asioita liittyen lopullisen ohjelman suoritukseen tietokoneen prosessorissa. Esimerkiksi C-kieltä käyttävä ohjelmoija ei näe, miten rekistereitä käytetään ohjelman suorituksessa eikä miten funktioiden kutsut on toteutettu matalalla tasolla.

Nyt on aika siirtyä ohjelmoimaan konekielellä, jotta saamme paremman käsityksen ohjelman matalan tason toiminnasta. Tutustumme tästä luvusta lähtien x86-konekielen käyttämiseen 64-bittisessä Linux-ohjelmoinnissa.
...
- Lippurekisteri `rflags` muodostuu biteistä, jotka antavat tietoa esimerkiksi viimeksi suoritetun komennon tuloksesta.
...
Seuraavassa on yksinkertainen 64-bittinen konekielinen Linux-ohjelma, joka tulostaa rivin tekstiä:
...
Ohjelma suorittaa kaksi systeemikutsua, joista ensimmäinen (`write`) näyttää viestin käyttäjälle ja toinen (`exit`) sulkee ohjelman. Rivin lopussa merkki `;` aloittaa kommentin, joka selventää rivin merkitystä.
...
<div class="note" markdown="1">
NASM on _assembleri_ eli kääntäjä, joka kääntää assembly-koodin tavumuotoiseksi konekieliseksi koodiksi. Lippu `-f elf64` tarkoittaa, että kääntäjä tuottaa 64-bittisen tiedoston ELF-formaatissa.
...
Nimiö `_start` tarkoittaa ohjelman alkukohtaa. Tämä on erityinen nimiö, jonka perusteella `ld` tietää linkityksessä, mistä ohjelman suoritus alkaa.
...
`bf 00 00 00 00` | `bf` <br> (siirrä 32-bittinen vakioarvo rekisteriin `edi`) | `00 00 00 00` <br> (0 = ohjelman paluuarvo)
`0f 05` | `0f 05` <br> (suorita systeemikutsu) | --
