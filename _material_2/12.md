...
Nyt meillä on tarvittava pohja siihen, että pystymme analysoimaan tarkasti C-kielisen ohjelman konekielistä käännöstä. Käytämme esimerkkinä seuraavaa ohjelmaa, joka laskee [Collatz-lukujonojen](../osa1/#ohjausrakenteet) yhteispituuden annettuun rajaan asti.
...
Ohjelmalle annetaan syötteenä kokonaisluku `t` ja ohjelma laskee summan funktion `collatz` arvoista välillä 1...`t`. Esimerkiksi:
...
Tässä tapauksessa laskennan tulos muodostuu seuraavasti:
...
Tarkastellaan ensin ohjelman käännöstä ilman erityistä kääntäjän optimointia eli tilanteessa, jossa käännöskomennossa ei ole käytetty optimointilippua. Voimme käyttää seuraavaa komentoa tuottamaan käännöksen assembly-version:
...
Tarkastellaan seuraavaksi saman ohjelman käännöstä optimointilipun `-O2` kanssa. Nyt kääntäjä tuottaa seuraavan assembly-koodin:
