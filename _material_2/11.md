...
Aliohjelma saa kuitenkin muuttaa vapaasti muita rekisterejä. Esimerkiksi äskeisessä aliohjelmassa `calc_sum` rekisterin `rdi` sisältö muuttuu aliohjelmassa, koska rekisteriä käytetään silmukan toteuttamiseen. Tämä ei ole ongelma, koska `rdi` ei ole säilytettävä rekisteri.
...
Rekisterin `rbp` hyötynä on kuitenkin, että muuttujien osoitteet suhteessa siihen säilyvät samoina koko aliohjelman ajan riippumatta siitä, miten aliohjelma käyttää pinoa. Esimerkiksi jos aliohjelma suorittaa komentoja `push` ja `pop`, rekisteri `rsp` muuttuu ja voisi olla sekavaa viitata sen kautta muuttujiin.
</div>
...
Kuitenkin pinomuistin varaaminen on tarpeellista silloin, kun kaikkea aliohjelman käsittelemää tietoa ei voi säilyttää rekistereissä tai johonkin tietoon täytyy pystyä viittaamaan muistiosoittimen kautta, jolloin se ei voi olla rekisterissä.

### Pinon tasaaminen

System V ABI -malli edellyttää, että kun aliohjelma kutsuu toista aliohjelmaa,
pino on tasattu niin, että pinorekisteri `rsp` osoittaa muistissa 16 tavun tavurajaan. Tämä tarkoittaa käytännössä sitä, että aliohjelman tulee lisätä pinoon tavuja jokin luvun 16 moninkerta ennen toisen aliohjelman kutsumista.
