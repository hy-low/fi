...
Tässä luvussa syvennymme tarkemmin pinon käyttämiseen aliohjelman toteutuksessa konekielisessä ohjelmoinnissa. Pinon avulla voidaan toteuttaa aliohjelman kutsuminen ja muistinvaraukset niin, että aliohjelmat pystyvät kutsumaan toisiaan eri tavoilla.

Tarkemmin ottaen tutustumme aliohjelmien toteuttamiseen 64-bittisessä Linux-ympäristössä. Tähän liittyy tiettyjä käytäntöjä, joita seuraamalla omia konekielisiä aliohjelmia voidaan kutsua myös muualta, kuten C-ohjelmasta.

## Pinon toiminta

_Pino_ (_stack_) on muistialue, jota käytetään aliohjelmakutsujen toteuttamiseen sekä aliohjelmien paikalliseen muistinvaraukseen. Ohjelman käynnistyessä käyttöjärjestelmä varaa ohjelmalle muistialueen, jota se voi käyttää pinona.

Konekieli tarjoaa tukea pinon käyttämiseen rekisterien ja komentojen kautta. Rekisteri `rsp` on _pinorekisteri_ (_stack pointer_), joka osoittaa pinon päälle. Komennot `push` ja `pop` lisäävät ja poistavat pinossa olevaa tietoa.
...
Pinon käyttäminen on yksi tapa vaihtaa kahden rekisterin arvot keskenään, mutta tämän voi tehdä tehokkaammin seuraavasti klassisella xor-trikillä:
...
Tämän menetelmän hienoutena on, ettei arvojen vaihtamiseen tarvitse käyttää ylimääräistä muistia rekisterien lisäksi.
</div>
...
Seuraava koodi puolestaan vastaa komentoa `pop rax`:
...
Aliohjelman toteutus komennoilla `call` ja `ret` perustuu pinon käyttämiseen. Komento `call` lisää ensin pinoon kutsukohdan paluuosoitteen ja siirtää sitten koodin suorituksen aliohjelman alkuun. Komento `ret` puolestaan hakee paluuosoitteen pinosta ja siirtää koodin suorituksen sinne.

Kutsukohdan paluuosoite tarkoittaa käytännössä sen komennon osoitetta, joka on koodissa komennon `call` jälkeen. Komento `call` muuttaa rekisteriä `rip` niin, että suoritus siirtyy aliohjelmaan, ja vastaavasti komento `ret` muuttaa rekisteriä `rip` niin, että suoritus siirtyy komentoa `call` seuraavaan komentoon.
...
Tässä komento `call swap_regs` lisää pinoon komennon `add rax, rbx` muistiosoitteen ja siirtää koodin suorituksen kohtaan `swap_regs`. Komento `ret` puolestaan hakee pinosta muistiosoitteen ja siirtää koodin suorituksen sinne.

Pinon vaiheet koodin suorituksen aikana:

![](../img/asm_stack_4.svg)

Tässä `next_rip` tarkoittaa kutsukohdan paluuosoitetta eli rekisterin `rip` arvoa aliohjelman suorituksen jälkeen.

## Aliohjelman käytännöt

Komentojen `call` ja `ret` avulla aliohjelman tiedonvälityksen voisi toteuttaa useilla tavoilla, mutta käyttöjärjestelmissä on tiettyjä käytäntöjä tähän liittyen. Tällaisia käytäntöjä ovat esimerkiksi, miten parametrit välitetään aliohjelmalle ja miten aliohjelma välittää paluuarvon kutsuvalle koodille.
...
<div class="note" markdown="1">
Perinteinen tapa konekielisessä ohjelmoinnissa on välittää _kaikki_ parametrit pinossa eikä käyttää rekisterejä. Tätä tapaa käytetään esimerkiksi 32-bittisessä Linux-ohjelmoinnissa. Etuna rekistereiden käyttämisessä on, että tämä tehostaa aliohjelman kutsumista pinon käyttämiseen verrattuna.

Huomaa, että rekisterien käyttötapa on _melkein_ sama aliohjelman ja systeemikutsun parametreissa mutta ei täysin sama. Erona on, että systeemikutsussa rekisterin `rcx` sijasta käytetään rekisteriä `r10`.
</div>
...
Jos aliohjelmalla on paluuarvo, se välitetään kutsuvalle koodille rekisterissä `rax`.
...
int main(void) {
    long sum = calc_sum(1, 100);
}
```

Tässä on suunnilleen vastaava konekielinen koodi:
...
Tämä aliohjelma toimii seuraavasti:

- Ensimmäinen parametri välitetään rekisterissä `rdi` ja toinen parametri välitetään rekisterissä `rsi`.
- Aliohjelma laskee lukuvälin summan rekisteriin `rax`, josta tulee aliohjelman paluuarvo. Aluksi `rax` saa arvon 0.
- Silmukan alussa koodi tarkastaa, onko `rdi`:n arvo suurempi kuin `rsi`:n arvo. Jos on, silmukka päättyy.
- Muuten `rdi`:n arvo lisätään `rax`:n arvoon, `rdi`:n arvo kasvaa yhdellä ja koodi hyppää takaisin silmukan alkuun.

<div class="note" markdown="1">
C-funktiossa parametrit `a` ja `b` ovat `int`-tyyppisiä, joten ne ovat 32-bittisiä eivätkä 64-bittisiä. Tämän takia tarkempi käännös konekieleksi olisi välittää arvot rekistereissä `edi` ja `esi`, jotka ovat rekisterien `rdi` ja `rsi` jälkipuoliskot.

Tässä tapauksessa tällä erolla on kuitenkin vähän merkitystä olettaen, että parametrit ovat positiivisia lukuja. Koska 32-bittisen rekisterin arvon asettaminen nollaa 64-bittisen rekisterin alkupuoliskon, esimerkiksi komennot `mov rdi, 1` ja `mov edi, 1` tuottavat saman tuloksen.

Tilanne on kuitenkin toinen, jos funktion parametri olisikin negatiivinen. Silloin 32-bittistä lukua ei voi muuttaa 64-bittiseksi nollaamalla rekisterin alkupuoliskoa eikä konekielinen koodi toimisi tarkoitetulla tavalla.
</div>
...
Yleinen käytäntö on tallentaa rekisteriin `rbp` (_base pointer_) rekisterin `rsp` sisältö aliohjelman alussa. Tällöin aliohjelma pystyy viittaamaan pinossa oleviin paikallisiin muuttujiin kätevästi rekisterin `rbp` kautta. Koska rekisteri `rbp` on säilytettävä rekisteri, tähän liittyen yleinen käytäntö on lisätä aliohjelman alussa pinoon kopio rekisterin `rbp` sisällöstä ja palauttaa se aliohjelman lopussa.

Tarkastellaan esimerkkinä C-kielistä funktiota, joka suorittaa laskutoimituksen:

```c
long calc_value(void) {
    long a = 123456789;
    long b = 987654321;
    return a * b;
}
...
Seuraavassa on vastaava konekielinen aliohjelma, joka käyttää rekisteriä `rbp` ja varaa pinosta tilaa muuttujille `a` ja `b`:
...
pinon tulee olla tasattu niin, että pinorekisteri `rsp` osoittaa muistissa 16 tavun rajaan eli rekisterissä `rsp` olevan muistiosoitteen tulee olla jaollinen 16:lla. Tämän takia aliohjelman tulee lisätä pinoon sopiva määrä tavuja ennen toisen aliohjelman kutsumista.

<div class="note" markdown="1">
Tämä vaatimus liittyy tiedon siirtämiseen pinon ja tiettyjen x86-prosessorin vektorirekistereiden välillä. Tiedon siirto on tehokkaampaa tai on mahdollista vain silloin, kun pino on tasattu 16 tavun rajaan.
</div>

Aliohjelman kutsuminen komennolla `call` aiheuttaa, että komento lisää pinoon 8-tavuisen kutsukohdan paluuosoitteen. Tämän takia pino ei ole tasattu aliohjelman suorituksen alkaessa. Kuitenkin jos aliohjelma lisää aluksi pinoon kopion 8-tavuisen rekisterin `rbp` sisällöstä, pino on jälleen tasattu.

Esimerkiksi seuraava koodi aliohjelman alussa tasaa myös pinon:
...
Tässä komento `push` lisää ensin pinoon 8 tavua tietoa, minkä jälkeen koodi varaa pinosta 16 tavua tilaa. Tämän koodin jälkeen rekisteri `rsp` osoittaa 16 tavun rajaan ja koodi voi kutsua turvallisesti toista aliohjelmaa.
...
Seuraava C-funktio laskee, montako kertaa luku esiintyy taulukossa:
...
Tässä on vastaavalla tavalla toimiva konekielinen aliohjelma:
...
Tässä tapauksessa parametreja on kolme, jolloin ne välitetään rekistereissä seuraavasti:

- `rdi`: osoitin taulukkoon (`array`)
- `rsi`: taulukon alkioiden määrä (`n`)
- `rdx`: haluttu taulukon alkio (`value`)

Konekielinen koodi laskee tuloksen rekisteriin `rax` ja käyttää rekisteriä `rcx` silmukan toteuttamisessa. Silmukka käy läpi taulukon alkiot ja tarkastaa jokaisen alkion kohdalla, onko se haluttu alkio.

Koska taulukko on `int`-tyyppinen, jokainen alkio vie tilaa neljä tavua. Tämän takia alkion sijainti muistissa saadaan laskettua kaavalla `rdi+4*rcx`, missä `rdi` on taulukon alkuosoite ja `rcx` on alkion indeksi.
...
fibo_calc:
    ...
    mov rbx, rax             ; store result to rbx
    pop rdi
    sub rdi, 2
    call fibo                ; calculate fibo(n - 2)
    add rax, rbx             ; sum recursive results
    pop rbx
    ret
```
...
Lopuksi koodi laskee yhteen rekisterien `rax` ja `rbx` arvot, joista tulee aliohjelman paluuarvo rekisteriin `rax`. Sen jälkeen täytyy vielä palauttaa rekisterin `rbx` alkuperäinen sisältö pinosta.
...
<div class="language-console highlighter-rouge"><div class="highlight"><pre class="highlight"><code>x/10gx $rsp
</code></pre></div></div>
