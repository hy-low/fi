...
Pino kasvaa muistissa oikealta vasemmalle, ja pinorekisteri `rsp` osoittaa pinon päälle. Esimerkiksi seuraava kuva esittää muistissa olevaa pinoa, johon on lisätty 64-bittiset luvut 1, 2 ja 3.
...
Komentojen `call` ja `ret` avulla aliohjelman voi toteuttaa useilla tavoilla, mutta käyttöjärjestelmissä on tiettyjä käytäntöjä aliohjelmien toteutukseen liittyen. Tällaisia käytäntöjä ovat esimerkiksi, miten parametrit välitetään aliohjelmalle ja miten aliohjelma välittää palautusarvon kutsuvalle koodille.

Linux-ohjelmoinnissa aliohjelmien toteutuksen käytäntönä on System V ABI -malli, johon tutustumme seuraavaksi. Tämän mallin seuraaminen omissa aliohjelmissa on hyödyllistä, koska silloin aliohjelmia pystyy kutsumaan odotetulla tavalla myös oman koodin ulkopuolelta.
...
Tässä aliohjelman ensimmäinen parametri (1)  välitetään rekisterissä `rdi` ja toinen parametri (10) välitetään rekisterissä `rsi`. Aliohjelma laskee lukuvälin summan rekisteriin `rax`, josta tulee aliohjelman palautusarvo.
...
Jos aliohjelma muuttaa säilytettävää rekisteriä, sen tulee palauttaa rekisterin sisältö ennalleen ennen aliohjelman päättymistä. Yleinen tapa toteuttaa tämä on kopioida rekisterin sisältö alussa pinoon ja palauttaa se pinosta lopuksi.
...
Tarkastellaan esimerkkenä C-kielistä funktiota, joka suorittaa laskutoimituksen:
...
<div class="note" markdown="1">
Tässä tapauksessa muuttujiin `a` ja `b` voisi viitata yhtä helposti rekisterin `rsp` kautta: muuttuja `a` on kohdassa `rsp` ja muuttuja `b` on kohdassa `rsp+8`.

Rekisterin `rbp` hyötynä on kuitenkin, että muuttujien osoitteet suhteessa siihen säilyvät samoina koko aliohjelman ajan riippumatta siitä, miten aliohjelma käyttää pinoa. Esimerkiksi jos aliohjelma suorittaa komentoja `push` ja `pop`, rekisterin `rsp` sisältö muuttuu ja voisi olla sekavaa viitata sen kautta muuttujiin.
</div>
...
Kuitenkin pinomuistin varaaminen on tarpeellista silloin, kun kaikkea aliohjelman käsittelemää tietoa ei voi säilyttää rekistereissä tai johonkin tietoon täytyy pystyä viittaamaan muistiosoitteen kautta, jolloin se ei voi olla rekisterissä.

### Pinon tasaaminen

System V ABI -mallissa aliohjelman kutsuessa toista aliohjelmaa
pinon tulee olla tasattu niin, että pinorekisteri `rsp` osoittaa muistissa 16 tavun tavurajaan. Tämä tarkoittaa käytännössä sitä, että aliohjelman tulee lisätä pinoon tavuja jokin luvun 16 moninkerta ennen toisen aliohjelman kutsumista.

<div class="note" markdown="1">
TODO: miksi näin?
</div>

Aliohjelman suorituksen alussa pino ei ole tasattu, koska pinossa on vain 8-tavuinen kutsukohdan paluuosoite. Kuitenkin jos aliohjelma lisää pinoon kopion 8-tavuisen rekisterin `rbp` sisällöstä, tämä tasaa pinon sisällön.
...
Jos aliohjelma ei kutsu mitään aliohjelmaa, sen ei tarvitse välittää pinon tasaamisesta.

## Rekursiivinen aliohjelma

Tarkastellaan seuraavaksi monimutkaisempaa tilannetta, jossa aliohjelma kutsuu itseään rekursiivisesti. Tavoitteena on tehdä aliohjelma, joka laskee rekursiivisesti halutun Fibonaccin luvun seuraavan funktion tyylisesti:
...
Voimme toteuttaa Fibonaccin luvun laskemisen konekielellä seuraavasti:
...
Tämä rekursiivinen toteutus on melko hidas suuremmilla parametreilla, koska samoja funktion arvoja lasketaan useita kertoja. Voimme kuitenkin tehostaa toteutusta dynaamisen ohjelmoinnin avulla lisäämällä koodiin taulukko `cache`, johon tallennetaan laskettuja tuloksia:
...
Nyt koodissa on uusi osio `fibo_cache`, joka tarkastaa taulukosta `cache`, onko tulos halutulle parametrille laskettu jo aiemmin. Jos on, koodi palauttaa suoraan tämän tuloksen. Muuten koodi laskee tuloksen rekursiivisesti kuten ennenkin ja tallentaa lasketun tuloksen taulukkoon `cache`.
